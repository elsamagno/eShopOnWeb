...
namespace Microsoft.eShopWeb.UnitTests.Builders
{
    public class OrderBuilder
    {
        ...
        public Order WithDefaultValues()
        {
            ...
            _order = new Order(TestBuyerId, new AddressBuilder().WithDefaultValues(),  OrderStatus.Pending, "default note", itemList);
            return _order;
        }

        public Order WithNoItems()
        {
            _order = new Order(TestBuyerId, new AddressBuilder().WithDefaultValues(), OrderStatus.Pending, "default note",  new List<OrderItem>());
            return _order;
        }

        public Order WithItems(List<OrderItem> items)
        {
            _order = new Order(TestBuyerId, new AddressBuilder().WithDefaultValues(), OrderStatus.Pending, "default note",  items);
            return _order;
        }
    }
}
